...
#----------------------------------------------(SELECT FILENAME FROM shippingnoticexml)-----------------------------------------------------------------------


		$conn = ConnectMySQL $user $pass $MySQLHost $database
		# Read all the records from table
		$query = "Select FileName from shippingnoticexml where ResponseFlag='False' Limit 1"
		...
		#---------------------------------------------(UPDATE THE VALUE OF FLAG IN shippingnoticexml)-----------------------------------------------------------------------
		 
		$conn = ConnectMySQL $user $pass $MySQLHost $database
		 
		# Read all the records from table
		$query = "Update plannermailing.shippingnoticexml set ResponseFlag = 'True' where  FileName = '$Value1'"

		$Rows = WriteMySQLQuerySetTrue $conn $query
		Write-Host $Rows "Updated value as True in Database in shippingnoticexml Table" 
		$frieght=0
		$args= $Value1,$frieght
